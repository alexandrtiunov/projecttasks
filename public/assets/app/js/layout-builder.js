var LayoutBuilder = function () {
    var e = {
        init: function () {
            ...
            $("#builder_export_html").click(function (a) {
                ...
                if (i) {
                    ...
                    e.startLoad({
                        ...
                        method: "POST",
                        data: {builder_export: 1, export_type: "partial", demo: $(r).data("demo"), purchase_code: i}
                    }).done(function (a) {
                        var o = JSON.parse(a);
                        if (o.message) e.stopWithNotify(o.message); else {
                            t.setItem("purchase_code", i);
                            var n = setInterval(function () {
                                $.ajax("index.php", {
                                    method: "POST",
                                    data: {builder_export: 1, builder_check: o.id}
                                }).done(function (t) {
                                    var a = JSON.parse(t);
                                    void 0 !== a && 1 === a.export_status && $("<iframe/>").attr({
                                        ...
                                        }, {type: "success"}), e.doneLoad(), clearInterval(n)
                                    }).appendTo(r)
                                })
                            }, 15e3)
                        }
            ...
            $("#builder_export_html_static").click(function (a) {
                ...
                if (i) {
                    ...
                    e.startLoad({
                        ...
                        method: "POST",
                        data: {builder_export: 1, export_type: "html", demo: $(r).data("demo"), purchase_code: i}
                    }).done(function (a) {
                        var o = JSON.parse(a);
                        if (o.message) e.stopWithNotify(o.message); else {
                            t.setItem("purchase_code", i);
            ...
            a = a || "danger", $.notify({title: "Verification failed", message: t}, {type: a}), e.doneLoad()
        }
    }, t = {
        expires_in: 3600, isVerified: function () {
        ...
        e.init(), $('[href^="#m_builder_"]').click(function (e) {
            var t = $(this).attr("href"), a = $('[name="builder_submit"]'), i = $('[name="builder[tab]"]');
            0 === $(i).length ? $("<input/>").attr("type", "hidden").attr("name", "builder[tab]").val(t).insertBefore(a) : $(i).val(t)
        }).each(function () {
            if ($(this).hasClass("active")) {
                var e = $(this).attr("href"), t = $('[name="builder_submit"]'), a = $('[name="builder[tab]"]');
